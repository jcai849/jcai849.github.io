---
title: A Review of Iteration with sparklyr
date: 2020-05-14
---
...
sparklyr can use this functionality through the `tbl_cache()`{.R} function to cache Spark dataframes in memory, as well as caching upon import with `memory=TRUE`{.R} as a formal parameter to `sdf_copy_to()`{.R}.
...
A standard form in R with non-optmimised initial value is given in listing [@lst:basicbab].
...
The first aspect that must be considered is that sparklyr works on Spark Data Frames;
`x`{.R} and `S`{.R} must be copied to Spark with the aforementioned `sdf_copy_to()`{.R} function.

The execution of the function in Spark is the next consideration, and sparklyr provides two means for this to occur;
`spark_apply()`{.R} evaluates arbitrary R code over an entire data frame.
...
In addition, very R-specific functions such as those from the `stats`{.R} and `matrix`{.R} core libraries are not able to be evaluated, as there is no Spark SQL cognate for them.
The SQL query generated by the methods can be accessed and "explained" through `show_query()`{.R} and `explain()`{.R} respectively;
When attempting to combine two Spark Data Frames in a single query without joining them, `show_query()`{.R} reveals that the Data Frame that is referenced through the `.data`{.R} variable is translated, but the other Data Frame has it's list representation passed through, which Spark SQL doesn't have the capacity to parse;
an example is given in listing [@lst:computer-no] (generated through listing [@lst:bad]), showing an attempt to create a new column from the difference between two seperate Data Frames

```{#lst:bad .R caption="Attempt in R to form new column from the difference between two separate Spark data frames `S`{.R} and `x`{.R}"}
...
to combine `S`{.R} and `x`{.R} in one dataframe, and iterate within
columns.
...
Thus, it is fair to conclude that sparklyr is incapable of sensible iteration of arbitrary R code beyond what maps directly to SQL; even with mutate, it is a very convoluted interface for attempting any iteration more complex than the Babylonian Method.
