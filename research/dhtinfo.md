---
title: Distributed Hash Table for the Dependency Graph
date: 2021-08-23
---
...
Currently, rather than sending messages directly from node-to-node, all messages are sent to queues named after chunks on a central Redis server, which routes the messages to nodes holding those chunks and performing blocking pops on the queues [@sanfilippo2009redis].
...
The principal issue with this approach is the high level of centralisation it entails, alongside the dependency and associated lack of behavioural controls over external software [@john2017survey].
...
A key motivator for the usage of a DHT transport layer, beyond the independence and decentralisation engendered, is also in enabling a distributed store of the dependency graph, as outlined in [Acyclic Dependency Digraphs for Immutable Chunks](depdag.html).
The advantages of a DHT in this respect include the ability to scale with the system, through CPU and memory load being spread through the system, as well as greater live fault-tolerance via in-memory replication [@lua2005p2p].
...
This means that there is greater direct control over the graph by the nodes hosting partitions of the graph, which is a shortcoming of Python's Dask; Dask has an explicit dependency graph, contained entirely within the master node, which leaves it out of reach of nodes that it affects directly [@rocklin2015dask].

# Overview of DHT's

A DHT is a key-value store with the capacity to be distributed over many disparate nodes [@balakrishnan2003p2p].
...
- Dynamic introduction and exit of nodes is a principal distinguishing feature of DHT's with respect to regular hash tables using nodes as buckets.
  ...
  DHT's have some hashing function that doesn't require system-wide remapping when nodes/buckets are added or removed [@thaler1996rendezvous].
- Nodes usually keep some routing table that is used to optimise lookups [@galuba2009p2p].
- Nodes typically have some node ID of the same length as keys (or some variation on keys, such as their SHA-1 hash), in order to allow for some direct comparison between them. Keys are usually hashed to particular nodes based on some measure of distance, choosing the node with the lowest distance to store the value on.
- Minimisation of route length and minimisation of degree are principal objectives in the analysis of DHT's, with most DHT's sporting \bigO{\log N} nodes contacted during lookups.
- All DHT's have some degree of susceptibility to Byzantine faults such as the Sybil attack, where a flood of new nodes can throw off the system; given that the **largeScaleR** system is expected to run in a non-adverserial environment, this consideration doesn't factor too heavily into choice of DHT algorithm [@douceur2002sybil] [@urdaneta2011dhtsec].
...
The following subsections describe the Chord and Kademlia algorithms, including discussion on suitability as a DHT message transport layer in **largeScaleR**, as well as some potential drawbacks, and suggestions for variations to ensure correctness.

## Chord

Chord builds on the consistent hashing algorithm, adding linear lookup and finger tables [@stoica2001chord].

Consistent hashing is a hashing scheme with a high probability of load balancing and minimisation of key remapping upon node exit and joining of the network, with only $\mathcal{O}(\frac{1}{N})$ expected key movement [@karger1997hash].
...
As this is $\mathcal{O}(N)$ in terms of route length, an additional stipulation is given that nodes carry a finger table, wherein addresses of nodes in exponentially increasing intervals on the ring are stored, and finger tables are consulted for routing instead of bare successors.
...
1. Node joins occur sequentially and force stabilisation procedures on both the successor and predecessor of the joining node.
...
2. Joining nodes broadcast their existence and ID recursively along finger tables, with the space of each recursive call bound by successors, forcing finger table fixes along all existing nodes.
...
Kademlia is a more complex algorithm than Chord, though it possesses certain features that make it more amenable to a large dynamic distributed system [@maymounkov2002kademlia].
Kademlia sees use by the Ethereum cryptocurrency, the Tox messaging protocol, and the gnunet file sharing system [@buterin2014ethereum; @alkhulaiwi2016tox; @wachs2014gnunet].
...
This is one major point of relevance to **largeScaleR**; these DHT's were originally written with semi-anonymous and potentially adversarial file-sharing in mind, while **largeScaleR** is intended to be run in a reasonably controlled environment where failing nodes can always be revived.

# Alternatives to Standard DHT Approaches
...
However, it is important to keep in mind that they are intended for extremely high-scale internet-based file-sharing applications, and **largeScaleR** can probably get away with an even simpler setup.

For instance, the routing table could instead be a list of all nodes, permitting joins and departures, and provide $\mathcal{O}(1)$ lookup cost, at the expense of an $\mathcal{O}(N)$ routing table.
...
Skip graph is a distributed version of the skiplist probabilistic data structure, with simple operations and impressive access costs [@aspnes2007skipgraph].
The skiplist algorithm which underpins it is made use of by Redis in [it's implementation](https://github.com/redis/redis/blob/unstable/src/t_zset.c) of ordered sets.

# Value and Key Descriptions
...
The dependency graph and DHT hosting it are separate, though depended upon, by the working **largeScaleR** system.
For all intents, the DHT may be accessed through a simple `read()`{.R} and `execute()`{.R} interface at the top level, with perhaps some middle layer communicating storage details and garbage collection information to the DHT.
The question remains of how much responsibility should be held by the DHT with respect to how much responsibility should be held by the operating **largeScaleR** system.
...
This information hiding aids in modularity and will hopefully result in less code changes necessary when changing out components in the future [@gamma1995design].
...
Attaining distributed consensus on changes to existing data is exceptionally difficult, though not impossible [@brewer1999cap; @gilbert2002brewer].
