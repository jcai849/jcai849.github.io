---
title: Message Queues for Communication in a Distributed Object System
date: 2020-08-19
---

# Introduction

A problem exists in the present architecture as described in [Distributed Object supplementary report](experiment-eager-dist-obj-supp.html).
...
The proposed amendments as described in [Proposed Information Structures](current-proposed-info-structures.html) provide some degree of amelioration, however the root of the issue in requiring full co-ordinating facilities from the master node is still not solved.

A potential solution exists in the use of message queues. Message queues are commonly used for inter-process communication, consisting of queues through which applications may communicate over [@curry2004message].
...
For example, the QNX OS makes heavy use of message queues for its microkernel architecture [@hildebrand1992qnx].
Tech companies Stack Overflow and flickr also use message queues from redis as central components of their infrastructure [@nolan2011flickr; @montrose2016stack].
In this platform, the flexibility of Redis lists and the availability of the rediscc package suggests the use of Redis in the implementation of message queues [@sanfilippo2009redis; @urbanek2020rediscc].
Alternatives include Apache ActiveMQ, Kafka, and Disque, among others [@snyder2011activemq; @garg2013kafka; @sanfilippo2016disque].
...
A very simple example is given by [@lst:msg-q-master; @lst:msg-q-worker], demonstrating the master and worker node routines respectively.

```{#lst:msg-q-master .R caption="Message queue master node"}
library(rediscc)
...
```{#lst:msg-q-worker .R caption="Message queue worker node"}
library(rediscc)
...
The master simply pushes serialised calls to the appropriate queue, and the worker loops reading messages from it's particular queue(s), unserialising and evaluating any messages.
...
In a similar vein, failure detection can be implemented through a concurrent "heartbeat" server, in the same manner as Hadoop [@white2012hadoop].
